<template>
  <div
    ref="multiselectRef"
    class="ct-multiselect"
    :class="{ 'ct-ms-disabled': disabled }"
  ...
  >
    ...
    >
      ...
      />
      <div v-else-if="forceLabel ?? label" class="ct-ms-label-text">
        {{ forceLabel ?? label }}
      </div>
      <div v-else class="ct-ms-placeholder">
        {{ placeholder ?? '' }}
      </div>
      <div v-if="clearable && value" class="ct-ms-clear" @click.stop="clear">
        <Cross class="ct-ms-clear-icon" />
      </div>
      <div v-if="caret" class="ct-ms-caret" :class="{ opened }"></div>
    </div>
    <Transition name="ct-ms-slide">
      <div v-show="opened" ref="menuRef" class="ct-ms-dropdown" :style="menuStyle">
        <div
          v-for="(l, index) in labels"
          :key="(l ?? index).toString()"
          class="ms-item"
          :class="{ 'ct-highlight': searchActive && index === searchIndex }"
          @click="select(index)"
        >
...
import CTInput from './CTInput.vue'
...
import { IMultiselectObj, IMultiselectOption, Keys } from '@app/types'
...
const searchRef = ref<InstanceType<typeof CTInput> | undefined>()
...
.ms-item.no-options {
  font-family: var(--ct-component-text, $font-text);
  font-weight: 400;
  font-size: 12px;
  padding: 6px 8px;
}
...
.multiselect-tooltip {
  font-family: var(--ct-component-title, $font-text);
  font-weight: 300;
  font-size: 12px;
  padding: 6px 8px;
  position: relative;
  color: var(--ct-tooltip-text-color, #212629);
  user-select: none;
  background-color: white;
  border-radius: 2px;
...
.ct-ms-slide-enter-from,
.ct-ms-slide-leave-to {
  transform: scaleY(0);
}
